...
class VoIPApp {
    ...
    handleMessage(message) {
        ...
        if (message === 'pinging' && this.status !== 'Calling') {
            ...
            this.setStatus('Idle');
        } else if (message.startsWith('volume local ')) {
            const volume = parseFloat(message.split(' ')[2]);
            this.localVolume = volume;
            this.drawBar(this.localCanvas, this.localVolume);
        } else if (message.startsWith('volume received ')) {
            const volume = parseFloat(message.split(' ')[2]);
            this.receivedVolume = volume;
            this.drawBar(this.receivedCanvas, this.receivedVolume);
        }
